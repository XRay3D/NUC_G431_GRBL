...
typedef struct {
    ...
    float position[AXIS::N]; // Where the interpreter considers the tool to be at this point in the code

    float coord_system[AXIS::N]; // Current work coordinate system (G54+). Stores offset from absolute machine
                                // position in mm. Loaded from EEPROM when called.
    float coord_offset[AXIS::N]; // Retains the G92 coordinate offset (work coordinates) relative to
