...
// constexpr auto CPU_MAP_ATMEGA328P;//  Arduino Uno CPU
...
constexpr auto HOMING_CYCLE_0 {(1 << AXIS::Z)};                    // REQUIRED: First move Z to clear workspace.
constexpr auto HOMING_CYCLE_1 {((1 << AXIS::X) | (1 << AXIS::Y))}; // OPTIONAL: Then move X,Y at the same time.
// constexpr auto HOMING_CYCLE_2{ }; // OPTIONAL: Uncomment and add axes mask to enable

// NOTE: The following are two examples to setup homing for 2-axis machines.
// constexpr auto HOMING_CYCLE_0{((1<<AXIS::X)|(1<<AXIS::Y)) }; // NOT COMPATIBLE WITH COREXY: Homes both X-Y in one cycle.

// constexpr auto HOMING_CYCLE_0{(1<<AXIS::X) }; // COREXY COMPATIBLE: First home X
// constexpr auto HOMING_CYCLE_1{(1<<AXIS::Y) }; // COREXY COMPATIBLE: Then home Y

// Number of homing cycles performed after when the machine initially jogs to limit switches.
// This help in preventing overshoot and should improve repeatability. This value should be one or
// greater.
constexpr auto N_HOMING_LOCATE_CYCLE = 1; // Integer (1-128)

// Enables single axis homing commands. $HX, $HY, and $HZ for X, Y, and Z-axis homing. The full homing
// cycle is still invoked by the $H command. This is disabled by default. It's here only to address
// users that need to switch between a two-axis and three-axis machine. This is actually very rare.
// If you have a two-axis machine, DON'T USE THIS. Instead, just alter the homing cycle for two-axes.
// constexpr auto HOMING_SINGLE_AXIS_COMMANDS;//  Default disabled. Uncomment to enable.

// After homing, Grbl will set by default the entire machine space into negative space, as is typical
// for professional CNC machines, regardless of where the limit switches are located. Uncomment this
// define to force Grbl to always set the machine origin at the homed location despite switch orientation.
// constexpr auto HOMING_FORCE_SET_ORIGIN;//  Uncomment to enable.

// Number of blocks Grbl executes upon startup. These blocks are stored in EEPROM, where the size
// and addresses are defined in settings.h. With the current settings, up to 2 startup blocks may
// be stored and executed in order. These startup blocks would typically be used to set the g-code
// parser state depending on user preferences.
constexpr auto N_STARTUP_LINE = 2; // Integer (1-2)
...
enum DECIMAL {
    COORDVALUE_INCH = 4, // Coordinate or position value in inches
    COORDVALUE_MM = 3,   // Coordinate or position value in mm
    RATEVALUE_INCH = 1,  // Rate or velocity value in in/min
    RATEVALUE_MM = 0,    // Rate or velocity value in mm/min
    RPMVALUE = 0,        // RPM value in rotations per min.
    SETTINGVALUE = 3,    // Decimals for floating point setting values
};
...
// constexpr auto ENABLE_M7;//  Disabled by default. Uncomment to enable.

// This option causes the feed hold input to act as a safety door switch. A safety door, when triggered,
// immediately forces a feed hold and then safely de-energizes the machine. Resuming is blocked until
// the safety door is re-engaged. When it is, Grbl will re-energize the machine and then resume on the
// previous tool path, as if nothing happened.
// constexpr auto ENABLE_SAFETY_DOOR_INPUT_PIN;//  Default disabled. Uncomment to enable.

// After the safety door switch has been toggled and restored, this setting sets the power-up delay
// between restoring the spindle and coolant and resuming the cycle.
constexpr auto SAFETY_DOOR_SPINDLE_DELAY = 4.0; // Float (seconds)
constexpr auto SAFETY_DOOR_COOLANT_DELAY = 1.0; // Float (seconds)

// Enable CoreXY kinematics. Use ONLY with CoreXY machines.
// IMPORTANT: If homing is enabled, you must reconfigure the homing cycle #defines above to
// constexpr auto HOMING_CYCLE_0=(1<<AXIS::X);  and constexpr auto HOMING_CYCLE_1=(1<<AXIS::Y);
// NOTE: This configuration option alters the motion of the X and Y axes to principle of operation
// defined at (http://corexy.com/theory.html). Motors are assumed to positioned and wired exactly as
// described, if not, motions may move in strange directions. Grbl requires the CoreXY A and B motors
// have the same steps per mm internally.
// constexpr auto COREXY;//  Default disabled. Uncomment to enable.
...
// constexpr auto INVERT_CONTROL_PIN_MASK=CONTROL_MASK; // Default disabled. Uncomment to disable.
// constexpr auto INVERT_CONTROL_PIN_MASK=((1<<CONTROL_SAFETY_DOOR_BIT)|(1<<CONTROL_RESET_BIT)); // Default disabled.
...
// constexpr auto INVERT_LIMIT_PIN_MASK=((1<<X_LIMIT_BIT)|(1<<Y_LIMIT_BIT)); // Default disabled. Uncomment to enable.
...
// constexpr auto INVERT_SPINDLE_ENABLE_PIN;//  Default disabled. Uncomment to enable.

// Inverts the selected coolant pin from low-disabled/high-enabled to low-enabled/high-disabled. Useful
// for some pre-built electronic boards.
// constexpr auto INVERT_COOLANT_FLOOD_PIN;//  Default disabled. Uncomment to enable.
// constexpr auto INVERT_COOLANT_MIST_PIN;//  Default disabled. Note: Enable M7 mist coolant in config.h
...
// constexpr auto FORCE_INITIALIZATION_ALARM;//  Default disabled. Uncomment to enable.
...
// constexpr auto DEBUG;//  Uncomment to enable. Default disabled.

// Configure rapid, feed, and spindle override settings. These values define the max and min
// allowable override values and the coarse and fine increments per command received. Please
// note the allowable values in the descriptions following each define.
constexpr auto DEFAULT_FEED_OVERRIDE = 100;         // 100%. Don't change this value.
constexpr auto MAX_FEED_RATE_OVERRIDE = 200;        // Percent of programmed feed rate (100-255). Usually 120% or 200%
constexpr auto MIN_FEED_RATE_OVERRIDE = 10;         // Percent of programmed feed rate (1-100). Usually 50% or 1%
constexpr auto FEED_OVERRIDE_COARSE_INCREMENT = 10; // (1-99). Usually 10%.
constexpr auto FEED_OVERRIDE_FINE_INCREMENT = 1;    // (1-99). Usually 1%.

constexpr auto DEFAULT_RAPID_OVERRIDE = 100; // 100%. Don't change this value.
constexpr auto RAPID_OVERRIDE_MEDIUM = 50;   // Percent of rapid (1-99). Usually 50%.
constexpr auto RAPID_OVERRIDE_LOW = 25;      // Percent of rapid (1-99). Usually 25%.
// constexpr auto RAPID_OVERRIDE_EXTRA_LOW=5; // *NOT SUPPORTED* Percent of rapid (1-99). Usually 5%.

constexpr auto DEFAULT_SPINDLE_SPEED_OVERRIDE = 100;   // 100%. Don't change this value.
constexpr auto MAX_SPINDLE_SPEED_OVERRIDE = 200;       // Percent of programmed spindle speed (100-255). Usually 200%.
constexpr auto MIN_SPINDLE_SPEED_OVERRIDE = 10;        // Percent of programmed spindle speed (1-100). Usually 10%.
constexpr auto SPINDLE_OVERRIDE_COARSE_INCREMENT = 10; // (1-99). Usually 10%.
constexpr auto SPINDLE_OVERRIDE_FINE_INCREMENT = 1;    // (1-99). Usually 1%.
...
constexpr auto REPORT_OVR_REFRESH_BUSY_COUNT = 20; // (1-255)
constexpr auto REPORT_OVR_REFRESH_IDLE_COUNT = 10; // (1-255) Must be less than or equal to the busy count
constexpr auto REPORT_WCO_REFRESH_BUSY_COUNT = 30; // (2-255)
constexpr auto REPORT_WCO_REFRESH_IDLE_COUNT = 10; // (2-255) Must be less than or equal to the busy count
...
constexpr auto ACCELERATION_TICKS_PER_SECOND = 100;
...
// constexpr auto MAX_STEP_RATE_HZ=30000; // Hz
...
constexpr auto TOOL_LENGTH_OFFSET_AXIS = AXIS::Z; // Default z-axis. Valid values are AXIS::X, AXIS::Y, or AXIS::Z.
...
// constexpr auto SPINDLE_PWM_MIN_VALUE=5; // Default disabled. Uncomment to enable. Must be greater than zero. Integer (1-255).
...
// constexpr auto USE_SPINDLE_DIR_AS_ENABLE_PIN;//  Default disabled. Uncomment to enable.
...
// constexpr auto SPINDLE_ENABLE_OFF_WITH_ZERO_SPEED;//  Default disabled. Uncomment to enable.
...
constexpr auto MINIMUM_JUNCTION_SPEED = 0.0; // (mm/min)

// Sets the minimum feed rate the planner will allow. Any value below it will be set to this minimum
// value. This also ensures that a planned motion always completes and accounts for any floating-point
// round-off errors. Although not recommended, a lower value than 1.0 mm/min will likely work in smaller
// machines, perhaps to 0.1mm/min, but your success may vary based on multiple factors.
constexpr auto MINIMUM_FEED_RATE = 1.0; // (mm/min)

// Number of arc generation iterations by small angle approximation before exact arc trajectory
// correction with expensive sin() and cos() calcualtions. This parameter maybe decreased if there
// are issues with the accuracy of the arc generations, or increased if arc execution is getting
// bogged down by too many trig calculations.
constexpr auto N_ARC_CORRECTION = 12; // Integer (1-255)
...
constexpr auto ARC_ANGULAR_TRAVEL_EPSILON = 5E-7; // Float (radians)
...
constexpr auto DWELL_TIME_STEP = 50; // Integer (1-255) (milliseconds)
...
// constexpr auto STEP_PULSE_DELAY=10; // Step pulse delay in microseconds. Default disabled.
...
// constexpr auto BLOCK_BUFFER_SIZE=16; // Uncomment to override default in planner.h.
...
// constexpr auto SEGMENT_BUFFER_SIZE=6; // Uncomment to override default in stepper.h.
...
// constexpr auto LINE_BUFFER_SIZE{80 }; // Uncomment to override default in protocol.h
...
// constexpr auto RX_BUFFER_SIZE=128; // (1-254) Uncomment to override defaults in serial.h
// constexpr auto TX_BUFFER_SIZE=100; // (1-254)
...
// constexpr auto ENABLE_SOFTWARE_DEBOUNCE;//  Default disabled. Uncomment to enable.

// Configures the position after a probing cycle during Grbl's check mode. Disabled sets
// the position to the probe target, when enabled sets the position to the start position.
// constexpr auto SET_CHECK_MODE_PROBE_TO_START;//  Default disabled. Uncomment to enable.
...
// constexpr auto HARD_LIMIT_FORCE_STATE_CHECK;//  Default disabled. Uncomment to enable.
...
// constexpr auto HOMING_AXIS_SEARCH_SCALAR=1.5; // Uncomment to override defaults in limits.c.
// constexpr auto HOMING_AXIS_LOCATE_SCALAR=10.0; // Uncomment to override defaults in limits.c.
...
// constexpr auto SETTINGS_RESTORE_ALL=SETTINGS_RESTORE_DEFAULTS | SETTINGS_RESTORE_PARAMETERS | SETTINGS_RESTORE_STARTUP_LINES | SETTINGS_RESTORE_BUILD_INFO;
...
// constexpr auto ALLOW_FEED_OVERRIDE_DURING_PROBE_CYCLES;//  Default disabled. Uncomment to enable.
...
// positive direction.
// constexpr auto PARKING_ENABLE;//  Default disabled. Uncomment to enable

// Configure options for the parking motion, if enabled.
constexpr auto PARKING_AXIS = AXIS::Z;          // Define which axis that performs the parking motion
constexpr auto PARKING_TARGET = -5.0;           // Parking axis target. In mm, as machine coordinate [-max_travel,0].
constexpr auto PARKING_RATE = 500.0;            // Parking fast rate after pull-out in mm/min.
constexpr auto PARKING_PULLOUT_RATE = 100.0;    // Pull-out/plunge slow feed rate in mm/min.
constexpr auto PARKING_PULLOUT_INCREMENT = 5.0; // Spindle pull-out and plunge distance in mm. Incremental distance.
...
// constexpr auto ENABLE_PARKING_OVERRIDE_CONTROL{ }; // Default disabled. Uncomment to enable
// constexpr auto DEACTIVATE_PARKING_UPON_INIT;//  Default disabled. Uncomment to enable.
...
// generate a solution.
// constexpr auto ENABLE_PIECEWISE_LINEAR_SPINDLE;//  Default disabled. Uncomment to enable.
...
constexpr auto N_PIECES = 4;               // Integer (1-4). Number of piecewise lines used in script solution.
constexpr auto RPM_MAX = 11686.4;          // Max RPM of model. $30 > RPM_MAX will be limited to RPM_MAX.
constexpr auto RPM_MIN = 202.5;            // Min RPM of model. $31 < RPM_MIN will be limited to RPM_MIN.
constexpr auto RPM_POINT12 = 6145.4;       // Used N_PIECES >=2. Junction point between lines 1 and 2.
constexpr auto RPM_POINT23 = 9627.8;       // Used N_PIECES >=3. Junction point between lines 2 and 3.
constexpr auto RPM_POINT34 = 10813.9;      // Used N_PIECES = 4. Junction point between lines 3 and 4.
constexpr auto RPM_LINE_A1 = 3.197101e-03; // Used N_PIECES >=1. A and B constants of line 1.
constexpr auto RPM_LINE_B1 = -3.526076e-1;
constexpr auto RPM_LINE_A2 = 1.722950e-2; // Used N_PIECES >=2. A and B constants of line 2.
constexpr auto RPM_LINE_B2 = 8.588176e+01;
constexpr auto RPM_LINE_A3 = 5.901518e-02; // Used N_PIECES >=3. A and B constants of line 3.
constexpr auto RPM_LINE_B3 = 4.881851e+02;
constexpr auto RPM_LINE_A4 = 1.203413e-01; // Used N_PIECES = 4. A and B constants of line 4.
constexpr auto RPM_LINE_B4 = 1.151360e+03;
...
// constexpr auto ENABLE_DUAL_AXIS =false; // Default disabled. Uncomment to enable.

// Select the one axis to mirror another motor. Only X and Y axis is supported at this time.
constexpr auto DUAL_AXIS_SELECT = AXIS::X; // Must be either AXIS::X or AXIS::Y
...
// travel of the other non-dual axis, i.e. if dual axis select is AXIS::X at 5.0%, then the fail
...
constexpr auto DUAL_AXIS_HOMING_FAIL_AXIS_LENGTH_PERCENT = 5.0; // Float (percent)
constexpr auto DUAL_AXIS_HOMING_FAIL_DISTANCE_MAX = 25.0;       // Float (mm)
constexpr auto DUAL_AXIS_HOMING_FAIL_DISTANCE_MIN = 2.5;        // Float (mm)
...
// constexpr auto DUAL_AXIS_CONFIG_CNC_SHIELD_CLONE;//  Uncomment to select. Comment other configs.
