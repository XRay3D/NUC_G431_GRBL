...
uint8_t gc_execute_line(char* line) {
    ...
    while (line[char_counter] != 0) { // Loop until no more g-code words in line.
        ...
        case 'G':
            ...
            if (BIT_IS_1(command_words, BIT(word_bit))) {
        ...
        default:
            ...
            case 'I':
                word_bit = WORD_I;
                gc_block.values.ijk[AXIS::X] = value;
                ijk_words |= (1 << AXIS::X);
                break;
            case 'J':
                word_bit = WORD_J;
                gc_block.values.ijk[AXIS::Y] = value;
                ijk_words |= (1 << AXIS::Y);
                break;
            case 'K':
                word_bit = WORD_K;
                gc_block.values.ijk[AXIS::Z] = value;
                ijk_words |= (1 << AXIS::Z);
                break;
            case 'L':
                word_bit = WORD_L;
            ...
            case 'X':
                word_bit = WORD_X;
                gc_block.values.xyz[AXIS::X] = value;
                axis_words |= (1 << AXIS::X);
                break;
            case 'Y':
                word_bit = WORD_Y;
                gc_block.values.xyz[AXIS::Y] = value;
                axis_words |= (1 << AXIS::Y);
                break;
            case 'Z':
                word_bit = WORD_Z;
                gc_block.values.xyz[AXIS::Z] = value;
                axis_words |= (1 << AXIS::Z);
                break;
            default:
                FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND);
            ...
            if (BIT_IS_1(value_words, BIT(word_bit))) {
    ...
    */
    ...
    if (BIT_IS_1(value_words, BIT(WORD_N))) {
        // Line number value cannot be less than zero (done) or greater than max line number.
        if (gc_block.values.n > MAX_LINE_NUMBER) {
            FAIL(STATUS_GCODE_INVALID_LINE_NUMBER);
        } // [Exceeds max line number]
    }
    // BIT_CLR(value_words,BIT(WORD_N)); // NOTE: Single-meaning value word. Set at end of error-checking.
    ...
    if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
        if (BIT_IS_0(value_words, BIT(WORD_F))) {
        ...
        if (gc_block.modal.feed_rate == FEED_RATE_MODE_INVERSE_TIME) { // = G93
            ...
            if (gc_state.modal.feed_rate == FEED_RATE_MODE_UNITS_PER_MIN) { // Last state is also G94
                if (BIT_IS_1(value_words, BIT(WORD_F))) {
                    if (gc_block.modal.units == UNITS_MODE_INCHES) {
                        gc_block.values.f *= MM_PER_INCH;
                    }
                } else {
    ...
    // BIT_CLR(value_words,BIT(WORD_F)); // NOTE: Single-meaning value word. Set at end of error-checking.

    // [4. Set spindle speed ]: S is negative (done.)
    if (BIT_IS_0(value_words, BIT(WORD_S))) {
        gc_block.values.s = gc_state.spindle_speed;
    }
// BIT_CLR(value_words,BIT(WORD_S)); // NOTE: Single-meaning value word. Set at end of error-checking.

// [5. Select tool ]: NOT SUPPORTED. Only tracks value. T is negative (done.) Not an integer. Greater than max tool value.
// BIT_CLR(value_words,BIT(WORD_T)); // NOTE: Single-meaning value word. Set at end of error-checking.
...
// [9. Override control ]: Not supported except for a Grbl-only parking motion override control.
#ifdef ENABLE_PARKING_OVERRIDE_CONTROL
    if (BIT_IS_1(command_words, BIT(MODAL_GROUP_M9))) { // Already set as enabled in parser.
        if (BIT_IS_1(value_words, BIT(WORD_P))) {
            if (gc_block.values.p == 0.0) {
                gc_block.modal.override = OVERRIDE_DISABLED;
            }
            BIT_CLR(value_words, BIT(WORD_P));
        }
    }
#endif
    ...
    if (gc_block.non_modal_command == NON_MODAL_DWELL) {
        if (BIT_IS_0(value_words, BIT(WORD_P))) {
    ...
    uint8_t idx;
    if (gc_block.modal.units == UNITS_MODE_INCHES) {
        for (idx = 0; idx < AXIS::N; idx++) { // Axes indices are consistent, so loop may be used.
            if (BIT_IS_1(axis_words, BIT(idx))) {
    ...
    float block_coord_system[AXIS::N];
    memcpy(block_coord_system, gc_state.coord_system, sizeof(gc_state.coord_system));
    if (BIT_IS_1(command_words, BIT(MODAL_GROUP_G12))) { // Check if called in block
        if (gc_block.modal.coord_select > N_COORDINATE_SYSTEM) {
            FAIL(STATUS_GCODE_UNSUPPORTED_COORD_SYS);
        } // [Greater than N sys]
        if (gc_state.modal.coord_select != gc_block.modal.coord_select) {
            if (!(Settings::read_coord_data(gc_block.modal.coord_select, block_coord_system))) {
    ...
    case NON_MODAL_SET_COORDINATE_DATA:
        ...
        if (BIT_IS_0(value_words, ((1 << WORD_P) | (1 << WORD_L)))) {
            FAIL(STATUS_GCODE_VALUE_WORD_MISSING);
        }                                        // [P/L word missing]
        ...
        if (gc_block.values.l != 20) {
            if (gc_block.values.l == 2) {
                if (BIT_IS_1(value_words, BIT(WORD_R))) {
                    FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND);
        ...
        BIT_CLR(value_words, (BIT(WORD_L) | BIT(WORD_P)));
        ...
        if (!Settings::read_coord_data(coord_select, gc_block.values.ijk)) {
            FAIL(STATUS_SETTING_READ_FAIL);
        } // [EEPROM read fail]
    ...
    default:
        ...
        if (axis_command != AXIS_COMMAND_TOOL_LENGTH_OFFSET) { // TLO block any axis command.
            if (axis_words) {
                for (idx = 0; idx < AXIS::N; idx++) { // Axes indices are consistent, so loop may be used to save flash space.
                    if (BIT_IS_0(axis_words, BIT(idx))) {
        ...
        case NON_MODAL_GO_HOME_1: // G30
            ...
            if (gc_block.non_modal_command == NON_MODAL_GO_HOME_0) {
                if (!Settings::read_coord_data(SETTING_INDEX_G28, gc_block.values.ijk)) {
                    FAIL(STATUS_SETTING_READ_FAIL);
                }
            } else { // == NON_MODAL_GO_HOME_1
                if (!Settings::read_coord_data(SETTING_INDEX_G30, gc_block.values.ijk)) {
                    FAIL(STATUS_SETTING_READ_FAIL);
                }
            }
            if (axis_words) {
        ...
        if (gc_block.modal.motion == MOTION_MODE_SEEK) {
            ...
            case MOTION_MODE_CCW_ARC:
                ...
                if (value_words & BIT(WORD_R)) { // Arc Radius Mode
                    BIT_CLR(value_words, BIT(WORD_R));
                    ...
                    */
                    ...
                    BIT_CLR(value_words, (BIT(WORD_I) | BIT(WORD_J) | BIT(WORD_K)));
    ...
    if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
        ...
        BIT_CLR(value_words, (BIT(WORD_N) | BIT(WORD_F)));
    } else {
        BIT_CLR(value_words, (BIT(WORD_N) | BIT(WORD_F) | BIT(WORD_S) | BIT(WORD_T))); // Remove single-meaning value words.
    }
    if (axis_command) {
        BIT_CLR(value_words, (BIT(WORD_X) | BIT(WORD_Y) | BIT(WORD_Z)));
    } // Remove axis words.
    if (value_words) {
    ...
    */
        ...
        if (axis_words && (axis_command == AXIS_COMMAND_MOTION_MODE)) {
            ...
            if (gc_state.modal.spindle == SPINDLE_ENABLE_CW) {
                if ((gc_state.modal.motion == MOTION_MODE_LINEAR) || (gc_state.modal.motion == MOTION_MODE_CW_ARC)
                    ...
                    if (BIT_IS_1(gc_parser_flags, GC_PARSER_LASER_DISABLE)) {
                    ...
                    if (BIT_IS_0(gc_parser_flags, GC_PARSER_LASER_DISABLE)) {
    ...
    // [4. Set spindle speed ]:
    if ((gc_state.spindle_speed != gc_block.values.s) || BIT_IS_1(gc_parser_flags, GC_PARSER_LASER_FORCE_SYNC)) {
        if (gc_state.modal.spindle != SPINDLE_DISABLE) {
#ifdef VARIABLE_SPINDLE
            if (BIT_IS_0(gc_parser_flags, GC_PARSER_LASER_ISMOTION)) {
                if (BIT_IS_1(gc_parser_flags, GC_PARSER_LASER_DISABLE)) {
...
// [9. Override control ]: NOT SUPPORTED. Always enabled. Except for a Grbl-only parking control.
    ...
    if (gc_state.modal.coord_select != gc_block.modal.coord_select) {
        ...
        memcpy(gc_state.coord_system, block_coord_system, AXIS::N * sizeof(float));
        system_flag_wco_change();
    }
    ...
    switch (gc_block.non_modal_command) {
    case NON_MODAL_SET_COORDINATE_DATA:
        Settings::write_coord_data(coord_select, gc_block.values.ijk);
        // Update system coordinate system if currently active.
        if (gc_state.modal.coord_select == coord_select) {
            memcpy(gc_state.coord_system, gc_block.values.ijk, AXIS::N * sizeof(float));
            system_flag_wco_change();
        }
        break;
    ...
    case NON_MODAL_GO_HOME_1:
        ...
        memcpy(gc_state.position, gc_block.values.ijk, AXIS::N * sizeof(float));
        break;
        ...
        if (axis_command == AXIS_COMMAND_MOTION_MODE) {
            ...
            if (gc_state.modal.motion == MOTION_MODE_LINEAR) {
                ...
                mc_arc(gc_block.values.xyz, pl_data, gc_state.position, gc_block.values.ijk, gc_block.values.r,
                    axis_0, axis_1, axis_linear, BIT_IS_1(gc_parser_flags, GC_PARSER_ARC_IS_CLOCKWISE));
            } else {
...
// upon a successful probing cycle, the machine position and the returned value should be the same.
    ...
    if (gc_state.modal.program_flow) {
        ...
        if (gc_state.modal.program_flow == PROGRAM_FLOW_PAUSED) {
            if (sys.state != STATE::CHECK_MODE) {
                system_set_exec_state_flag(EXEC::FEED_HOLD); // Use feed hold for program pause.
