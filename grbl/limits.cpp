...
// number in bit position, i.e. AXIS::Z is (1<<2) or bit 2, and AXIS::Y is (1<<1) or bit 1.
...
#endif
    if (BIT_IS_0(settings.flags, BITFLAG_INVERT_LIMIT_PINS)) {
        pin ^= LIMIT_MASK;
    }
    if (pin) {
        uint8_t idx;
...
extern "C" void ISR_LIMIT_INT_vect() // DEFAULT: Limit pin change interrupt process.
{
...
extern "C" void ISR_LIMIT_INT_vect() {
    if (!(WDTCSR & (1 << WDIE))) {
        WDTCSR |= (1 << WDIE);
    }
}
extern "C" void ISR_WDT_vect() // Watchdog timer ISR
{
    WDTCSR &= ~(1 << WDIE);         // Disable watchdog timer.
    if (sys.state != STATE::ALARM) { // Ignore if already in alarm state.
        if (!(sys_rt_exec_alarm)) {
            // Check limit pin state.
            if (limits_get_state()) {
                mc_reset();                                   // Initiate system kill.
...
void limits_go_home(uint8_t cycle_mask) {
    ...
    uint8_t step_pin[AXIS::N];
#if ENABLE_DUAL_AXIS
    uint8_t step_pin_dual;
    uint8_t dual_axis_async_check;
    int32_t dual_trigger_position;
    #if (DUAL_AXIS_SELECT == AXIS::X)
    float fail_distance = (-DUAL_AXIS_HOMING_FAIL_AXIS_LENGTH_PERCENT / 100.0) * settings.max_travel[AXIS::Y];
    #else
    float fail_distance = (-DUAL_AXIS_HOMING_FAIL_AXIS_LENGTH_PERCENT / 100.0) * settings.max_travel[AXIS::X];
    #endif
    ...
    for (idx = 0; idx < AXIS::N; idx++) {
        ...
        if ((idx == A_MOTOR) || (idx == B_MOTOR)) {
            step_pin[idx] = (get_step_pin_mask(AXIS::X) | get_step_pin_mask(AXIS::Y));
        }
#endif

        if (BIT_IS_1(cycle_mask, BIT(idx))) {
    ...
    do {
        ...
        homing_rate *= sqrt(n_active_axis); // [sqrt(AXIS::N)] Adjust so individual axes all move at homing rate.
        ...
        do {
            ...
            if (sys_rt_exec_state & (EXEC::SAFETY_DOOR | EXEC::RESET | EXEC::CYCLE_STOP)) {
                ...
                if (approach && (rt_exec & EXEC::CYCLE_STOP)) {
