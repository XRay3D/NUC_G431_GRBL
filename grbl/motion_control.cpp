...
void mc_line(float* target, plan_line_data_t* pl_data) {
    ...
    if (BIT_IS_1(settings.flags, BITFLAG_SOFT_LIMIT_ENABLE)) {
...
void mc_arc(float* target, plan_line_data_t* pl_data, float* position, float* offset, float radius,
    ...
    if (segments) {
        ...
        if (pl_data->condition & PL_COND_FLAG_INVERSE_TIME) {
            ...
            BIT_CLR(pl_data->condition, PL_COND_FLAG_INVERSE_TIME); // Force as feed absolute mode over arc segments.
        }
...
uint8_t mc_probe_cycle(float* target, plan_line_data_t* pl_data, uint8_t parser_flags) {
    ...
    uint8_t is_probe_away = BIT_IS_1(parser_flags, GC_PARSER_PROBE_IS_AWAY);
    uint8_t is_no_error = BIT_IS_1(parser_flags, GC_PARSER_PROBE_IS_NO_ERROR);
...
void mc_parking_motion(float* parking_target, plan_line_data_t* pl_data) {
    ...
    if (plan_status) {
        BIT_SET(sys.step_control, STEP_CONTROL_EXECUTE_SYS_MOTION);
        BIT_CLR(sys.step_control, STEP_CONTROL_END_MOTION); // Allow parking motion to execute, if feed hold is active.
        ...
        BIT_CLR(sys.step_control, STEP_CONTROL_EXECUTE_SYS_MOTION);
        protocol_exec_rt_system();
    }
}
#endif
...
void mc_reset() {
    ...
    if (BIT_IS_0(sys_rt_exec_state, EXEC::RESET)) {
        system_set_exec_state_flag(EXEC::RESET);
        ...
        if ((sys.state & (STATE::CYCLE | STATE::HOMING | STATE::JOG)) || (sys.step_control & (STEP_CONTROL_EXECUTE_HOLD | STEP_CONTROL_EXECUTE_SYS_MOTION))) {
