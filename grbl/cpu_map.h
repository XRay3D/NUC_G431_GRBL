...
#if defined(CPU_MAP_ATMEGA328P) // (Arduino Uno) Officially supported by Grbl.
...
inline char
    ...
    // Define stepper driver enable/disable output pin.
    #define STEPPERS_DISABLE_DDR  GPIOA->ODR
    ...
    // Define homing/hard limit switch input pins and limit interrupt vectors.
    // NOTE: All limit bit pins must be on the same port, but not on a port with other input pins (CONTROL).
    #define LIMIT_DDR             GPIOA->IDR
    #define LIMIT_PIN             GPIOA->IDR
    #define LIMIT_PORT            GPIOA->IDR
    #define X_LIMIT_BIT           10 // Uno Digital Pin 9
    #define Y_LIMIT_BIT           11 // Uno Digital Pin 10
    #ifdef VARIABLE_SPINDLE          // Z Limit pin and spindle enabled swapped to access hardware PWM on Pin 11.
        #define Z_LIMIT_BIT 4        // Uno Digital Pin 12
    #else
    ...
    // Define user-control controls (cycle start, reset, feed hold) input pins.
    // NOTE: All CONTROLs pins must be on the same port and not on a port with other input pins (limits).
    // FIXME   #define CONTROL_DDR             DDRC
    // FIXME   #define CONTROL_PIN             PINC
    // FIXME   #define CONTROL_PORT            PORTC
    #define CONTROL_RESET_BIT       0      // Uno Analog Pin 0
    #define CONTROL_FEED_HOLD_BIT   1      // Uno Analog Pin 1
    #define CONTROL_CYCLE_START_BIT 2      // Uno Analog Pin 2
    // FIXME   #define CONTROL_SAFETY_DOOR_BIT 1     // Uno Analog Pin 1 NOTE: Safety door is shared with feed hold. Enabled by config define.
    // FIXME   #define CONTROL_INT             PCIE1 // Pin change interrupt enable pin
    // FIXME   #define CONTROL_INT_vect        PCINT1_vect
    // FIXME   #define CONTROL_PCMSK           PCMSK1 // Pin change interrupt register
    // FIXME   #define CONTROL_MASK            ((1 << CONTROL_RESET_BIT) | (1 << CONTROL_FEED_HOLD_BIT) | (1 << CONTROL_CYCLE_START_BIT) | (1 << CONTROL_SAFETY_DOOR_BIT))
    // FIXME   #define CONTROL_INVERT_MASK     CONTROL_MASK // May be re-defined to only invert certain control pins.

    // Define probe switch input pin.
    #define PROBE_DDR               GPIOB->IDR
    #define PROBE_PIN               GPIOB->IDR
    #define PROBE_PORT              GPIOB->IDR
