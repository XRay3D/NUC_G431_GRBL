...
void Settings::store_startup_line(uint8_t n, char* line) {
#ifdef FORCE_BUFFER_SYNC_DURING_EEPROM_WRITE
    protocol_buffer_synchronize(); // A startup line may contain a motion and be executing.
#endif
    uint32_t addr = n * (LINE_BUFFER_SIZE + 1) + EEPROM_ADDR_STARTUP_BLOCK;
    eeprom::memcpy_to_eeprom_with_checksum(addr, (uint8_t*)line, LINE_BUFFER_SIZE);
}

// Method to store build info into EEPROM
// NOTE: This function can only be called in IDLE state.
void Settings::store_build_info(char* line) {
    // Build info can only be stored when state is IDLE.
    eeprom::memcpy_to_eeprom_with_checksum(EEPROM_ADDR_BUILD_INFO, (uint8_t*)line, LINE_BUFFER_SIZE);
}

// Method to store coord data parameters into EEPROM
void Settings::write_coord_data(uint8_t coord_select, float* coord_data) {
#ifdef FORCE_BUFFER_SYNC_DURING_EEPROM_WRITE
    protocol_buffer_synchronize();
#endif
    uint32_t addr = coord_select * (sizeof(float) * AXIS::N + 1) + EEPROM_ADDR_PARAMETERS;
    eeprom::memcpy_to_eeprom_with_checksum(addr, (uint8_t*)coord_data, sizeof(float) * AXIS::N);
}

// Method to store Grbl global settings struct and version number into EEPROM
// NOTE: This function can only be called in IDLE state.
void write_global_settings() {
    eeprom::put_char(0, SETTINGS_VERSION);
    eeprom::memcpy_to_eeprom_with_checksum(EEPROM_ADDR_GLOBAL, (uint8_t*)&settings, sizeof(settings_t));
}

// Method to restore EEPROM-saved Grbl global settings back to defaults.
void Settings::restore(uint8_t restore_flag) {
    ...
    if (restore_flag & SETTINGS_RESTORE_PARAMETERS) {
        ...
        for (idx = 0; idx <= SETTING_INDEX_NCOORD; idx++) {
            Settings::write_coord_data(idx, coord_data);
        }
    }
    ...
    if (restore_flag & SETTINGS_RESTORE_STARTUP_LINES) {
        ...
        eeprom::put_char(EEPROM_ADDR_STARTUP_BLOCK + 1, 0); // Checksum
#endif
#if N_STARTUP_LINE > 1
        eeprom::put_char(EEPROM_ADDR_STARTUP_BLOCK + (LINE_BUFFER_SIZE + 1), 0);
        eeprom::put_char(EEPROM_ADDR_STARTUP_BLOCK + (LINE_BUFFER_SIZE + 2), 0); // Checksum
#endif
    }

    if (restore_flag & SETTINGS_RESTORE_BUILD_INFO) {
        eeprom::put_char(EEPROM_ADDR_BUILD_INFO, 0);
        eeprom::put_char(EEPROM_ADDR_BUILD_INFO + 1, 0); // Checksum
    }
}

// Reads startup line from EEPROM. Updated pointed line string data.
uint8_t Settings::read_startup_line(uint8_t n, char* line) {
    uint32_t addr = n * (LINE_BUFFER_SIZE + 1) + EEPROM_ADDR_STARTUP_BLOCK;
    if (!(eeprom::memcpy_from_eeprom_with_checksum((uint8_t*)line, addr, LINE_BUFFER_SIZE))) {
        // Reset line with default value
        line[0] = 0; // Empty line
        Settings::store_startup_line(n, line);
        return (false);
    }
    return (true);
}
...
// Reads startup line from EEPROM. Updated pointed line string data.
uint8_t Settings::read_build_info(char* line) {
    if (!(eeprom::memcpy_from_eeprom_with_checksum((uint8_t*)line, EEPROM_ADDR_BUILD_INFO, LINE_BUFFER_SIZE))) {
        // Reset line with default value
        line[0] = 0; // Empty line
...
uint8_t Settings::read_coord_data(uint8_t coord_select, float* coord_data) {
    uint32_t addr = coord_select * (sizeof(float) * AXIS::N + 1) + EEPROM_ADDR_PARAMETERS;
    if (!(eeprom::memcpy_from_eeprom_with_checksum((uint8_t*)coord_data, addr, sizeof(float) * AXIS::N))) {
        // Reset with default zero vector
        clear_vector_float(coord_data);
        Settings::write_coord_data(coord_select, coord_data);
        return (false);
    }
    return (true);
}
...
uint8_t read_global_settings() {
    ...
    uint8_t version = eeprom::get_char(0);
    if (version == SETTINGS_VERSION) {
        // Read settings-record and check checksum
        if (!(eeprom::memcpy_from_eeprom_with_checksum((uint8_t*)&settings, EEPROM_ADDR_GLOBAL, sizeof(settings_t)))) {
            return (false);
        }
    } else {
        return (false);
...
uint8_t Settings::store_global_setting(uint8_t parameter, float value) {
    if (value < 0.0) {
    ...
    if (parameter >= AXIS_SETTINGS_START_VAL) {
        ...
        while (set_idx < AXIS_N_SETTINGS) {
            if (parameter < AXIS::N) {
                ...
                set_idx++;
                // If axis index greater than AXIS::N or setting index greater than number of axis settings, error out.
        ...
        case 20:
            if (int_value) {
                if (BIT_IS_0(settings.flags, BITFLAG_HOMING_ENABLE)) {
