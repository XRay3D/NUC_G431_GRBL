...
typedef struct {
    int32_t position[AXIS::N];        // The planner position of the tool in absolute steps. Kept separate
                                     // from g-code position for movements requiring multiple line motions,
                                     // i.e. arcs, canned cycles, and backlash compensation.
    float previous_unit_vec[AXIS::N]; // Unit vector of previous path line segment
...
/* Add a new linear movement to the buffer. target[AXIS::N] is the signed, absolute target position
...
uint8_t plan_buffer_line(float* target, plan_line_data_t* pl_data) {
    ...
    int32_t target_steps[AXIS::N], position_steps[AXIS::N];
    float unit_vec[AXIS::N], delta_mm;
    uint8_t idx;

    // Copy position data based on type of motion being planned.
    if (block->condition & PL_COND_FLAG_SYSTEM_MOTION) {
#ifdef COREXY
        position_steps[AXIS::X] = system_convert_corexy_to_x_axis_steps(sys_position);
        position_steps[AXIS::Y] = system_convert_corexy_to_y_axis_steps(sys_position);
        position_steps[AXIS::Z] = sys_position[AXIS::Z];
#else
    ...
    block->steps[A_MOTOR] = labs((target_steps[AXIS::X] - position_steps[AXIS::X]) + (target_steps[AXIS::Y] - position_steps[AXIS::Y]));
    block->steps[B_MOTOR] = labs((target_steps[AXIS::X] - position_steps[AXIS::X]) - (target_steps[AXIS::Y] - position_steps[AXIS::Y]));
#endif
...
// NOTE: Computes true distance from converted step values.
        ...
        if (idx == A_MOTOR) {
            delta_mm = (target_steps[AXIS::X] - position_steps[AXIS::X] + target_steps[AXIS::Y] - position_steps[AXIS::Y]) / settings.steps_per_mm[idx];
        } else if (idx == B_MOTOR) {
            delta_mm = (target_steps[AXIS::X] - position_steps[AXIS::X] - target_steps[AXIS::Y] + position_steps[AXIS::Y]) / settings.steps_per_mm[idx];
        } else {
    ...
    if ((block_buffer_head == block_buffer_tail) || (block->condition & PL_COND_FLAG_SYSTEM_MOTION)) {
        ...
        float junction_unit_vec[AXIS::N];
...
#ifdef COREXY
        if (idx == AXIS::X) {
            pl.position[AXIS::X] = system_convert_corexy_to_x_axis_steps(sys_position);
        } else if (idx == AXIS::Y) {
            pl.position[AXIS::Y] = system_convert_corexy_to_y_axis_steps(sys_position);
        } else {
