...
// and timer accuracy.  Do not alter these settings unless you know what you are doing.
    ...
    #define AMASS_LEVEL1 (SystemCoreClock / 8000) // Over-drives ISR (x2). Defined as SystemCoreClock/(Cutoff frequency in Hz)
    #define AMASS_LEVEL2 (SystemCoreClock / 4000) // Over-drives ISR (x4)
    #define AMASS_LEVEL3 (SystemCoreClock / 2000) // Over-drives ISR (x8)
...
void st_wake_up() {
    ...
    if (BIT_IS_1(settings.flags, BITFLAG_INVERT_ST_ENABLE)) {
...
// Initialize step pulse timing from settings. Here to ensure updating after re-writing.
    ...
    LL_TIM_EnableIT_UPDATE(TIM2); // FIXME TIMSK1 |= (1 << OCIE1A);
}

// Stepper shutdown
void st_go_idle() {
    // Disable Stepper Driver Interrupt. Allow Stepper Port Reset Interrupt to finish, if active.
    LL_TIM_DisableIT_UPDATE(TIM2); // FIXME TIMSK1 &= ~(1 << OCIE1A);                                       // Disable Timer1 interrupt
    // FIXME TCCR1B = (TCCR1B & ~((1 << CS12) | (1 << CS11))) | (1 << CS10); // Reset clock to no prescaling.
    busy = false;

    // Set stepper driver idle state, disabled or enabled, depending on settings and circumstances.
    bool pin_state = false; // Keep enabled.
    if (((settings.stepper_idle_lock_time != 0xff) || sys_rt_exec_alarm || sys.state == STATE::SLEEP) && sys.state != STATE::HOMING) {
...
// Then pulse the stepping pins
    ...
    TIM4->ARR = st.step_pulse_time; // FIXME TCNT0 = st.step_pulse_time; // Reload Timer0 counter
    LL_TIM_EnableCounter(TIM4);     // FIXME TCCR0B = (1 << CS01);       // Begin Timer0. Full speed, 1/8 prescaler
    ...
    if (st.exec_segment == NULL) {
        ...
        if (segment_buffer_head != segment_buffer_tail) {
            ...
            TIM2->CCR1 = st.exec_segment->cycles_per_tick; // FIXME OCR1A = st.exec_segment->cycles_per_tick;
            ...
            st.steps[AXIS::X] = st.exec_block->steps[AXIS::X] >> st.exec_segment->amass_level;
            st.steps[AXIS::Y] = st.exec_block->steps[AXIS::Y] >> st.exec_segment->amass_level;
            st.steps[AXIS::Z] = st.exec_block->steps[AXIS::Z] >> st.exec_segment->amass_level;
#endif
...
#endif
            system_set_exec_state_flag(EXEC::CYCLE_STOP); // Flag main program for cycle end
...
#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
    st.counter_x += st.steps[AXIS::X];
#else
    st.counter_x += st.exec_block->steps[AXIS::X];
#endif
    if (st.counter_x > st.exec_block->step_event_count) {
        st.step_outbits |= (1 << X_STEP_BIT);
#if defined(ENABLE_DUAL_AXIS) && (DUAL_AXIS_SELECT == AXIS_X)
...
#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
    st.counter_y += st.steps[AXIS::Y];
#else
    st.counter_y += st.exec_block->steps[AXIS::Y];
#endif
    if (st.counter_y > st.exec_block->step_event_count) {
        st.step_outbits |= (1 << Y_STEP_BIT);
#if defined(ENABLE_DUAL_AXIS) && (DUAL_AXIS_SELECT == AXIS_Y)
        st.step_outbits_dual = (1 << DUAL_STEP_BIT);
#endif
...
#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
    st.counter_z += st.steps[AXIS::Z];
#else
    st.counter_z += st.exec_block->steps[AXIS::Z];
#endif
...
extern "C" void ISR_TIMER0_OVF_vect() {
    ...
    // FIXME TCCR0B = 0; // Disable Timer0 to prevent re-entering this interrupt when it's not needed.
...
void st_generate_step_dir_invert_masks() {
    ...
    for (idx = 0; idx < AXIS::N; idx++) {
        if (BIT_IS_1(settings.step_invert_mask, BIT(idx))) {
            step_port_invert_mask |= get_step_pin_mask(idx);
        }
        if (BIT_IS_1(settings.dir_invert_mask, BIT(idx))) {
    ...
    // NOTE: Dual axis invert uses the AXIS::N bit to set step and direction invert pins.
    if (BIT_IS_1(settings.step_invert_mask, BIT(AXIS::N))) {
        step_port_invert_mask_dual = (1 << DUAL_STEP_BIT);
    }
    if (BIT_IS_1(settings.dir_invert_mask, BIT(AXIS::N))) {
...
void st_prep_buffer() {
    ...
    if (BIT_IS_1(sys.step_control, STEP_CONTROL_END_MOTION)) {
        return;
    }
...
#ifdef VARIABLE_SPINDLE
            BIT_SET(sys.step_control, STEP_CONTROL_UPDATE_SPINDLE_PWM); // Force update whenever updating block.
#endif
        }
        ...
        */
        ...
        if (prep_segment->n_step == 0) {
            if (sys.step_control & STEP_CONTROL_EXECUTE_HOLD) {
                ...
                BIT_SET(sys.step_control, STEP_CONTROL_END_MOTION);
#ifdef PARKING_ENABLE
                if (!(prep.recalculate_flag & PREP_FLAG_PARKING)) {
...
float st_get_realtime_rate() {
    if (sys.state & (STATE::CYCLE | STATE::HOMING | STATE::HOLD | STATE::JOG | STATE::SAFETY_DOOR)) {
